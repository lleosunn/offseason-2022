...
@Autonomous
public class newgyroxdistance extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (sensorRange.getDistance(DistanceUnit.INCH) > 12) { //go backward
                double power = 0.2 + ((sensorRange.getDistance(DistanceUnit.INCH) - 12) * 0.04);
            ...
            } else if (sensorRange.getDistance(DistanceUnit.INCH) < 11) { //go forward
                double power = 0.2 + ((11 - sensorRange.getDistance(DistanceUnit.INCH)) * 0.04);
                double turnpower = 0.3;
                if (getAngle() < -1){ //turn left
                    tl.setPower(power + (0.01 * getAngle()));
                    tr.setPower(-(power - (0.01 * getAngle())));
            ...
            if (sensorRange2.getDistance(DistanceUnit.INCH) < 11) { //go right
                double power = 0.4 + ((11 - sensorRange2.getDistance(DistanceUnit.INCH)) * 0.04);
                double turnpower = 0.3;
            ...
            } else if (sensorRange2.getDistance(DistanceUnit.INCH) > 12) { //go left
                double power = 0.4 + ((sensorRange2.getDistance(DistanceUnit.INCH) - 12) * 0.04);
                double turnpower = 0.3;
                if (getAngle() < -1){ //turn left
                    tl.setPower(-(power - (0.01 * getAngle())));
                    tr.setPower(-(power - (0.01 * getAngle())));
