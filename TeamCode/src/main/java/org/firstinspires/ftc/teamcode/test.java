...
public class test extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        sensorRange1 = hardwareMap.get(DistanceSensor.class, "right");
        sensorRange2 = hardwareMap.get(DistanceSensor.class, "back");
        ...
        while (opModeIsActive()) {
            ...
            if (sensorRange1.getDistance(DistanceUnit.INCH) > 12) { //go forward
                double power = 0.2 + ((sensorRange1.getDistance(DistanceUnit.INCH) - 12) * 0.06);
            ...
            } else if (sensorRange1.getDistance(DistanceUnit.INCH) < 11) { //go backward
                double power = 0.2 + ((11 - sensorRange1.getDistance(DistanceUnit.INCH)) * 0.06);
                if (getAngle() < -1) { //turn left
                    tl.setPower(-(power - (0.01 * getAngle())));
                    tr.setPower((power + (0.01 * getAngle())));
                    bl.setPower(-(power - (0.01 * getAngle())));
            ...
            if (sensorRange2.getDistance(DistanceUnit.INCH) > 12) { //go left
                double power = 0.4 + ((sensorRange1.getDistance(DistanceUnit.INCH) - 12) * 0.15);
                if (getAngle() < -1){ //turn left
                    tl.setPower(-(power - (0.01 * getAngle())));
                    tr.setPower(-(power - (0.01 * getAngle())));
                    bl.setPower((power + (0.01 * getAngle())));
